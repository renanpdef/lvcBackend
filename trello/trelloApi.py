...
def adicionarPedido(pedido, nome, email, tel, PR, MR, DR, PV, GA, PM, FO):
    url = "https://api.trello.com/1/cards"
    querystring = {"name": "Pedido " + str(pedido),
                   "desc": "Cliente: \n "+
                            "nome: "+str(nome)+"\n"+
                            "email: "+str(email)+"\n" +
                            "telefone: "+str(tel)+"\n\n"+
...
def obterStatusPedidos():
    ...
    for i in range(0,len(jsonResponse)):
        if(str(jsonResponse[i]["type"]) == "updateCard"):
            nomePedido = str(jsonResponse[i]["data"]["card"]["name"])
            if(not nomePedido in statusPedido and "listAfter" in str(jsonResponse[i]["data"])):
                status = idsStatus[str(jsonResponse[i]["data"]["listAfter"]["id"])]
                statusPedido[nomePedido] = status
    return statusPedido
